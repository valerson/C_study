...
void rotateC(int *pxL, int *pyL, int *pxR, int *pyR)
{
    int xC = (int)(*pxL + *pxR) / 2, yC = (int)(*pyL + *pyR) / 2;
    int h = *pyL - *pyR, w = *pxR - *pxL;
    *pxL = xC - h * 0.5;
    *pyL = yC + w * 0.5;
    *pxR = xC + h * 0.5;
    *pyR = yC - w * 0.5;
...
{
    int xL, yL, xR, yR;
    int *pxL = &xL, *pyL = &yL, *pxR = &xR, *pyR = &yR;
    scanf("%d%d%d%d", pxL, pyL, pxR, pyR);
    rotateC(pxL, pyL, pxR, pyR);
    printf("%d %d %d %d", xL, yL, xR, yR);
    return 0;
}